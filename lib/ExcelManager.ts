...
export class ExcelManager {
	...
	public async createWorksheet(name: string): Promise<string> {
		return this.withRetry(async () => {
			...
			await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}/range(address='${headerRange}')`)
				.patch({
					values: [headers]
				});
	...
	public async deleteRow(range: string, yearRange: string): Promise<void> {
		return this.withRetry(async () => {
			try {
				...
				await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}/range(address='${range}')/delete`)
					.post({ shift: "Up" });
