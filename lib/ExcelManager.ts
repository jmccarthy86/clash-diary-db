...
export class ExcelManager {
  ...
  private async initializeWorksheet(worksheetIdOrName: string): Promise<void> {
  ...
  public async setWorksheet(worksheetIdOrName: string): Promise<void> {
    ...
    if (typeof worksheetIdOrName === 'string' && worksheetIdOrName.match(/^[0-9A-F]{8}-[0-9A-F]{4}-[4][0-9A-F]{3}-[89AB][0-9A-F]{3}-[0-9A-F]{12}$/i)) {
  ...
  private async findWorksheetIdByName(name: string): Promise<string | null> {
    return this.withRetry(async () => {
      const response = await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets`)
        .get();
  ...
  private async lockRange(range: string): Promise<void> {
    await this.withRetry(async () => {
      await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}/range(address='${range}')/cell(row=0,column=0)`)
        .patch({
          values: [['LOCKED']]
        });
      this.log('Lock', { range });
  ...
  public async editRow(rowId: number, newData: any[]): Promise<void> {
    const range = `A${rowId}:Z${rowId}`;
    return this.withRetry(async () => {
      try {
        await this.lockRange(range);
        await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}/range(address='${range}')`)
          .patch({
            values: [newData]
          });
        this.log('EditRow', { rowId, newData });
      } finally {
  ...
  public async createNewRow(newData: any[]): Promise<number> {
    return this.withRetry(async () => {
      const usedRange = await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}/usedRange`)
        .get();
      const lastRowAddress = usedRange.address.split('!')[1].split(':')[1];
      const lastRow = parseInt(lastRowAddress.replace(/[A-Z]/g, ''));
      const newRowId = lastRow + 1;
      const newRowRange = `A${newRowId}:Z${newRowId}`;
      
      try {
        await this.lockRange(newRowRange);
        await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}/range(address='${newRowRange}')`)
          .patch({
            values: [newData]
          });
        this.log('CreateNewRow', { newRowId, newData });
        return newRowId;
      } finally {
  ...
  public async readRow(rowId: number): Promise<any[]> {
    return this.withRetry(async () => {
      const response = await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}/range(address='A${rowId}:Z${rowId}')`)
        .get();
  ...
  public async findRowByValue(columnIndex: number, value: any): Promise<number | null> {
    return this.withRetry(async () => {
      const usedRange = await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}/usedRange`)
        .get();
      const lastRow = parseInt(usedRange.address.split('!')[1].split(':')[1].replace(/[A-Z]/g, ''));
  ...
  public async getSheet(): Promise<any> {
    return this.withRetry(async () => {
      const response = await this.client.api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}`)
        .get();
  ...
  public async getUsedRangeValues(): Promise<any[][]> {
    return this.withRetry(async () => {
      const response = await this.client
        .api(`/users/${this.userId}/drive/items/${this.workbookId}/workbook/worksheets/${this.worksheetId}/usedRange`)
        .get();
  ...
  public async searchGetAllRowsMatching(columnIndex: number, value: any): Promise<any[][]> {
    return this.withRetry(async () => {
      ...
      this.log('SearchGetAllRowsMatching', { columnIndex, value, matchCount: matchingRows.length });
      return matchingRows;
    });
  }
}
...
export async function createExcelManager(workbookId: string, worksheetIdOrName: string, userId: string): Promise<ExcelManager> {
